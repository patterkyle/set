#lang racket

(require rackunit
         rackunit/text-ui
         "prelude.rkt"
         "logic.rkt")

(define-test-suite prelude-tests
  (test-begin
    (def apple 'banana)
    (check-equal? apple 'banana)

    (def ([cranberry 'dragonfruit]))
    (check-equal? cranberry 'dragonfruit)

    (def ([e 'frapple]
          [grapefruit 'h]))
    (check-equal? e          'frapple)
    (check-equal? grapefruit 'h)

    (def ([i 'jackfruit]
          [k 'lemon]
          [m 'n]))
    (check-equal? i 'jackfruit)
    (check-equal? k 'lemon)
    (check-equal? m 'n)

    (def ([o 'p]
          [q 'r]
          [s 't]
          [u 'v]
          [w 'x]))
    (check-equal? o 'p)
    (check-equal? q 'r)
    (check-equal? s 't)
    (check-equal? u 'v)
    (check-equal? w 'x)

    (def (foo bar)
      bar)
    (check-equal? (foo 'bar) 'bar)

    (def (foo* [baz 'baz])
      baz)
    (check-equal? (foo*) 'baz)))

(define-test-suite logic-tests
  (check-equal? (card 'one 'squiggle 'open 'green)
                (card 'one 'squiggle 'open 'green))
  (check-not-equal? (card 'one 'squiggle 'open 'green)
                    (card 'one 'squiggle 'open 'purple))
  (check-true (= 81 (vector-length (make-deck))))
  (check-true (all-equal? #(1 1 1)))
  (check-false (all-equal? #(1 2 3)))
  (check-true (all-equal? (vector (card 'two 'oval 'striped 'red)
                                  (card 'two 'oval 'striped 'red))))
  (check-false (all-equal? (vector (card 'two 'oval 'striped 'red)
                                   (card 'two 'diamond 'striped 'red))))
  (check-true (all-distinct? #(1 2 3 0)))
  (check-false (all-distinct? #(1 2 3 2)))
  (check-true (all-distinct? (vector (card 'two 'oval 'striped 'red)
                                     (card 'two 'oval 'striped 'green))))
  (check-false (all-distinct? (vector (card 'two 'oval 'striped 'red)
                                      (card 'two 'oval 'striped 'red))))
  (check-true (potential-set? (vector (card 'two 'oval 'striped 'red)
                                      (card 'two 'oval 'striped 'green))))
  (check-false (makes-set? (vector (card 'two 'oval 'striped 'red)
                                   (card 'two 'oval 'striped 'green))))
  (check-true (makes-set? (vector (card 'two 'oval 'striped 'red)
                                  (card 'two 'oval 'striped 'green)
                                  (card 'two 'oval 'striped 'purple))))
  (check-true (makes-set? (vector (card 'one 'squiggle 'striped 'red)
                                  (card 'two 'oval 'solid 'green)
                                  (card 'three 'diamond 'open 'purple))))
  (check-true (not (makes-set? (vector (card 'three 'oval 'striped 'red)
                                       (card 'two 'oval 'striped 'green)
                                       (card 'two 'oval 'striped 'purple))))))

(run-tests prelude-tests)
(run-tests logic-tests)
